...
### New Features
- **Capital** web feature (`AB.startCapital()`, `AB.makeCapitalViewController()`)
- **WKWebView improvements**: file download support and `target="_blank"` link handling

### Improvements
- Comprehensive error handling for all ProximityReader and processor error cases with user-facing messages
- Fiserv account linking and session initialization errors now surface to the user
- Resolved Swift 6 strict concurrency warnings when integrating with projects using `StrictConcurrency`
- Restored native arm64 iOS Simulator support for `Autobooks.xcframework`; CocoaPods integrators can remove any `EXCLUDED_ARCHS = arm64` simulator workaround
- Updated triPOS to v5.2.2

### Bug Fixes
- Fix `startTapToPay` completion block never firing
- Fix first-time setup stuck on onboarding when processor errors occur
- Fix stale state after fatal error on SDK reinvocation
- Fix crash during Worldpay payment processing
- Fix Fiserv payments failing after returning from background
- Fix reliability issues when reinvoking `makeTapToPayController()` multiple times
- Fix `disableDuplicateDetection` not resetting between SDK invocations
- Fix occasional crash on the Tap to Pay loading screen when network is slow or unavailable
- Fix Worldpay payment crashes on declined or interrupted card reads
- Fix crash during product synchronization
- Fix products from a previously signed-in user appearing on the items grid under a new account until the user tapped Charge
- Fix iOS 26.3.1 crash when dismissing the items list
- Fix crash on returning to the host app after opening the items grid, backgrounding, and then closing Tap to Pay
- Fix Tap to Pay showing a generic error instead of a clear "App Not Active" message when the app is backgrounded during a payment
- Fix Worldpay Tap to Pay showing "No Internet Connection" when the payment service is temporarily unavailable
- Fix Tap to Pay showing a generic error instead of "Service Temporarily Unavailable" when a payment service returns a server error
- Fix the Items tray getting stuck underneath the keyboard after cancelling a card read and returning to the amount screen
